...
/**
 * This thread runs in the background while the user has our program in the
 * foreground, and handles sending mDNS queries and processing incoming mDNS
 * packets.
 * 
 * @author simmons
 */
public class NetThread extends Thread {
	...
	private static final byte[] MDNS_ADDR = new byte[] { (byte) 224, (byte) 0,
			(byte) 0, (byte) 251 };
	 ...
	 * @param activity
	 */
	public NetThread(WifiManager wifiManager, MulticastLock multicastLock,
			ControlDeviceActivity obj) {
	...
	private NetworkInterface getFirstWifiOrEthernetInterface() {
		...
		try {
			...
			while (networkInterfaces.hasMoreElements()) {
				networkIntrfc = (NetworkInterface) networkInterfaces
						.nextElement();
	...
	/**
	 * The main network loop. Multicast DNS packets are received, processed, and
	 * sent to the UI.
	 * 
	 * This loop may be interrupted by closing the multicastSocket, at which
	 * time any commands in the commandQueue will be processed.
	 */
	@Override
	public void run() {
		...
		while (i++ < 500) {
			...
			try {
				...
				Log.v(TAG, "received: hostname "
						+ response.getAddress().getHostName() + " address:"
						+ response.getSocketAddress() + "multi ");
				HostIP = response.getAddress().getHostName();
				receivedStringData = new String(response.getData(), 0,
	...
	private void decodePacketToReadable() {
		byte[] pattern = new byte[] { 0x11, (byte) 0x94 };// {(byte) 0xC0, 0x0C,
															// (byte) 0xC0,
															// 0x2F};
		offset = 47;// BytesIndexOf(recvData, pattern, 0) + 4;//14;
		HostName = readName();
		Log.d(TAG, " Info-- " + HostName);// bytesToString(recvData, 13,
											// (int)readByte()));
		// offset =12;
