...
public class PacketHandler {
	...
	public boolean ConnectionRequested = false, ReadyToSend = false,
			stopRXThread = true;
	...
	public void StartSocketController() {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					Inet4_Addr = (Inet4Address) Inet4Address
							.getByName(s_IP_Addr);
				...
				Log.d("CC3000HomeAutomationSocket",
						"Attempting to connect socket..."
								+ Inet4_Addr.getHostAddress());
				...
				while (ConnectionRequested) {
					...
					if (pSocOpen && !SocOpen)
						Log.d("CC3000HomeAutomationSocket",
								"Socket was closed.");
					if (!pSocOpen && SocOpen)
						Log.d("CC3000HomeAutomationSocket", "Socket open.");
					...
					if (!SocOpen || !SocConnected) {
						try {
							...
							Log.d("CC3000HomeAutomationSocket", "IOexception  "
									+ e.getMessage());
						}
					}
				...
				if (!Soc.isClosed()) {
					try {
						Log.d("CC3000HomeAutomationSocket",
								"Socket disconnect requested. Attempting to close socket.");
						Soc.close();
						instream.close();
						outstream.close();
						Isconnected = false;
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while (ConnectionRequested) {
					if (Soc != null && RX_hdnlr != null && instream != null)
						if (Soc.isConnected() && !Soc.isClosed()) {
							if (Isconnected) {
								try {
									...
									msg = aControlDeviceActivity.handler
											.obtainMessage();
									msg.what = 1;
									aControlDeviceActivity.handler
											.sendMessage(msg);
								}
							}
						}
	...
	public void Send(final byte[] outpktbuffer) {
		if (Soc.isConnected() && ConnectionRequested && !Soc.isClosed()) {
			...
			Log.d("CC3000HomeAutomationSocket",
					"inside send backgroung - Data sent");
		}
	}
}
